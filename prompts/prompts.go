...
- Exact JSON record shapes: inv[] items are {name,desc,props?,state?}; env.objs[] objects are {name,props?,state?} with no desc; npcs[] are {name,disp,know?,goal} with no desc; puzzles[] are {name,type,desc,status,hints}; nouns[] are {noun,phrase,desc}.
- Exact JSON types and names: inv[].props and env.objs[].props are arrays of strings, never objects or maps, for example ["conductive"]; desc is allowed only on inv items, env, puzzles, and nouns; state is a string; conds, know, hints, win, loss, and solved_puzzles are arrays of strings; exits map strings to strings.
